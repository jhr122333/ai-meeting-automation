...
const buildDemoStaffReport = (filename: string) => `## 💬 누가 뭐라고 했나요
...
const buildDemoExecReport = (filename: string) => `## 회의 결과 보고
...
회의록 자동화 도구 도입 필요성을 검토하고, 부서별 보고 체계 일원화 방안을 논의함.
...
| 구분 | 내용 | 비고 |
|---|---|---|
| 현황 진단 | 부서별 회의록 작성 방식 상이로 산출물 품질 편차 발생 | 개선 시급 |
| 문제 원인 | 보고 대상별 상세본/요약본 이중 작성으로 인한 업무 과부하 | 주당 평균 2시간 낭비 |
| 개선 방향 | AI 기반 자동 보고서 생성 시스템 도입을 통한 프로세스 표준화 | 즉시 검토 가능 |

---

### 3. 의사결정 사항

1. 차기 회의부터 음성 파일 업로드 방식을 표준 프로세스로 채택
2. 실무진용·경영진용 보고서 동시 생성 체계 도입 추진
3. 부서 간 일정 재확인 후 전체 공지 예정 (담당: 이수민, 3/18)
...
| 리스크 | 심각도 | 대응 방안 |
|---|---|---|
| 부서별 보고 포맷 혼재 | 중 | 표준 템플릿 배포 및 가이드라인 수립 |
| 회의 후 공유 지연 | 상 | 자동 생성 즉시 배포 프로세스 도입 |
| 액션 아이템 누락 | 중 | AI 자동 추출 로직 적용으로 보완 |
...
| 담당자 | 업무 | 기한 |
|---|---|---|
| 김현우 | 1차 실행안 정리 및 팀 내 공유 | 3/15 |
| 이수민 | 관련 부서 일정 재확인 및 공지 | 3/18 |
| 박지훈 | 리스크 항목 최신화 후 슬랙 게시 | 3/19 |`;
